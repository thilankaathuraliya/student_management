...
import { createTeacher, getTeachers } from "../api/endpoints";
...
export default function TeachersScreen() {
  ...
  const load = useCallback(
    async (nextPage: number, append: boolean) => {
      ...
      try {
        setError(null);
        const res = await getTeachers(token, nextPage, PAGE_SIZE);
  ...
  async function onSave() {
    ...
    setError(null);
    try {
      await createTeacher(token, name.trim());
      setModalOpen(false);
      setName("");
      await load(1, false);
    } catch (e) {
      if (e instanceof ApiError && e.status === 401) await logout();
      setError(e instanceof Error ? e.message : "Could not create teacher");
    } finally {
      setSaving(false);
    }
  }
